   ...
   : mCertList(GTK_TREE_VIEW(gtk_tree_view_new()))
{
   ...
   GtkWidget *view = gtk_button_new_with_mnemonic(_("_View Certificate"));
   gtk_widget_show(GTK_WIDGET(view));
   AddSensitiveWidget(view);
   g_signal_connect(view, "clicked", G_CALLBACK(OnView), this);

   GtkButton *select = Util::CreateButton(GTK_STOCK_OK, _("_Select"));
   gtk_widget_show(GTK_WIDGET(select));
   GTK_WIDGET_SET_FLAGS(select, GTK_CAN_DEFAULT);
   SetForwardButton(select);
   g_signal_connect(G_OBJECT(select), "clicked", G_CALLBACK(OnSelect), this);

   GtkButton *help = GetHelpButton();
   GtkWidget *actionArea = Util::CreateActionArea(help, GTK_BUTTON(view),
                                                  select, GetCancelButton(),
                                                  NULL);
   gtk_widget_show(actionArea);
   gtk_box_pack_start(GTK_BOX(vbox), actionArea, false, true, 0);
   gtk_button_box_set_child_secondary(GTK_BUTTON_BOX(actionArea), view, true);
 ...
 *-----------------------------------------------------------------------------
 *
 * cdk::ScCertDlg::OnSelect --
 *
 *      Click handler for the select button.  Emit the select signal.
 *
 * Results:
 *      None
 *
...
void
ScCertDlg::OnSelect(GtkButton *button, // IN/UNUSED
                    gpointer userData) // IN
{
   ...
   if (gtk_tree_selection_count_selected_rows(
          gtk_tree_view_get_selection(that->mCertList)) > 0) {
      that->select();
   }
}
 ...
 *-----------------------------------------------------------------------------
 *
 * cdk::ScCertDlg::OnView --
 *
 *      Click handler for the View Certificate button.  Display the
 *      certificate's details to the user.
 *
 * Results:
 *      None
 *
...
void
ScCertDlg::OnView(GtkButton *button, // IN
                  gpointer userData) // IN
{
   ScCertDlg *that = reinterpret_cast<ScCertDlg*>(userData);
   ASSERT(that);

   // This deletes itself when bututon is destroyed.
