...
Cryptoki::Module::GetSlotNames(std::set<Util::string> &slots) // IN/OUT
{
   ...
   for (std::list<CK_SLOT_ID>::iterator i = slotIds.begin();
        i != slotIds.end(); i++) {
      rv = mFuncs.C_GetSlotInfo(*i, &info);
      if (rv == CKR_OK) {
         /*
         ...
         slots.insert(g_strchomp(tmpLabel));
         g_free(tmpLabel);
      }
   }
}
                                   ...
                                   STACK_OF(X509_NAME) *CAs) // IN
{
   ...
   while (true) {
      ...
      GByteArray *id = g_byte_array_sized_new(attrs[1].ulValueLen);
...
Cryptoki::Session::GetPrivateKey(const X509 *cert) // IN
{
   ...
   if (mNeedLogin) {
      for (int i = 0; i < 3; i++) {
         char *pin = mModule->GetCryptoki()->requestPin(mLabel, cert);
         if (!pin) {
            Log("No PIN specified for [%s]\n", mLabel.c_str());
            return NULL;
         }
         rv = funcs.C_Login(mSession, CKU_USER, (unsigned char *)pin, strlen(pin));
         // Overwrite pin before freeing!
         memset(pin, 0, strlen(pin));
         g_free(pin);
         if (rv == CKR_OK) {
            break;
         }
         Warning("C_Login attempt %d failed: %#lx [%s]\n",
                 i, rv, mLabel.c_str());
      }
      if (rv != CKR_OK) {
         return NULL;
